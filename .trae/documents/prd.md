...
| 角色 | 注册方式 | 核心权限 |
|------|----------|----------|
| 普通用户 | 邮箱 + 密码（默认必选）；OAuth（GitHub / Google，可选，需管理员配置密钥后开启） | 创建目标、规划航程、记录日志、查看仪表盘、密码重置 |
...
| 仪表盘 | 今日聚焦 | 展示当前优先级最高的 1 个目标与 3 个待办任务；无目标时显示引导创建入口 |
...
| 登录/注册 | 密码重置 | 邮箱发送重置链接，链接有效期 30 分钟，一次性使用 |
...
用户在登录页点击「忘记密码」 → 输入注册邮箱 → 系统生成一次性重置令牌（30 分钟有效）并发送邮件 → 用户点击邮件链接进入重置页 → 设置新密码 → 旧令牌失效，跳转登录。
