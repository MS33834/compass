  ...
  const trendBg =
    trendDirection === 'increasing'
      ...
      : trendDirection === 'decreasing'
        ...
        : 'bg-gray-100';
  const trendIcon =
    trendDirection === 'increasing' ? '📈' : trendDirection === 'decreasing' ? '📉' : '➡️';
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-md">
      ...
      <svg width={width} height={height} className="mx-auto">
        ...
        {points.map((point, index) => (
          <g key={index}>
            ...
            />
            <title>{`${new Date(point.timestamp).toLocaleDateString()}: ${point.score}分`}</title>
          </g>
        ))}
      ...
      <div className="mt-4 text-center text-sm text-slate-600">
        共{dataPoints.length}次测评 | 时间范围：
        {new Date(dataPoints[0]?.timestamp || 0).toLocaleDateString()} -{' '}
        {new Date(dataPoints[dataPoints.length - 1]?.timestamp || 0).toLocaleDateString()}
      </div>
    </div>
  );
}
