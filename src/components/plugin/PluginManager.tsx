...
export function PluginManager() {
  ...
  const filteredPlugins = filter === 'all' ? plugins : plugins.filter(p => p.type === filter);
      ...
      <div className="flex items-center justify-center min-h-64">
        <div className="text-center">
          ...
          <p className="text-slate-600">
            {locale === 'zh' ? '加载插件系统...' : 'Loading plugins...'}
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-slate-600 text-lg">
          {locale === 'zh'
            ? '管理系统插件，扩展功能模块'
            : 'Manage system plugins and extend functionality'}
        </p>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {(['all', 'assessment', 'training', 'ui', 'core'] as const).map(type => (
          ...
          >
            {type === 'all' ? (locale === 'zh' ? '全部' : 'All') : getTypeIcon(type)}{' '}
            {getTypeLabel(type)}
          </button>
        ))}
      </div>
        ...
        {filteredPlugins.map(plugin => (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              ...
              <span
                className={`px-2.5 py-1 rounded-full text-xs font-medium border ${getTypeColor(plugin.type)}`}
              >
            ...
            {plugin.dependencies.length > 0 && (
              <div className="mb-3">
                ...
                {plugin.dependencies.map(dep => (
                  <span
                    key={dep}
                    className="inline-block px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-xs mr-1"
                  >
                    {dep}
                  </span>
                ))}
            ...
            <div className="flex items-center justify-between pt-3 border-t border-slate-100">
              <div className="flex items-center gap-2">
                <span
                  className={`w-2.5 h-2.5 rounded-full ${activePlugins.includes(plugin.id) ? 'bg-green-500' : 'bg-slate-300'}`}
                ></span>
                <span className="text-xs text-slate-500">
                  {activePlugins.includes(plugin.id)
                    ? locale === 'zh'
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div className="bg-slate-50 rounded-xl p-3">
                ...
                <p
                  className={`font-medium ${activePlugins.includes(selectedPlugin.id) ? 'text-green-600' : 'text-slate-500'}`}
                >
