...
function generateInterpretation(traitKey: string, tScore: number): ScoreInterpretation {
  ...
  const interpretations: Record<
    string,
    Record<ScoreLevel, { title: string; description: string }>
  > = {
    O: {
      veryLow: {
        title: '极度务实保守',
...
export function generateBigFiveReport(traits: TraitResult[]) {
  ...
  const personalityType =
    PERSONALITY_COMBINATIONS[combinationKey as keyof typeof PERSONALITY_COMBINATIONS];
    ...
    if (key) {
      if (key === 'N') {
        ...
        careerRecommendations.push(
          ...CAREER_RECOMMENDATIONS[key as keyof typeof CAREER_RECOMMENDATIONS]
        );
      }
    }
  });
  ...
  return {
    ...
    recommendations: {
      ...
      personalGrowth: traitAnalyses
        .filter(a => a && a.fullInterpretation?.detailed?.growthAreas)
        .flatMap(a => a?.fullInterpretation?.detailed?.growthAreas || [])
        .slice(0, 6),
      relationships: traitAnalyses
        .filter(a => a && a.fullInterpretation?.detailed?.relationships)
        .map(a => a?.fullInterpretation?.detailed?.relationships)[0],
      workStyle: traitAnalyses
        .filter(a => a && a.fullInterpretation?.detailed?.workStyle)
        .map(a => a?.fullInterpretation?.detailed?.workStyle)[0],
    },
    strengths: traitAnalyses
      .filter(a => a && a.fullInterpretation?.detailed?.strengths)
      .flatMap(a => a?.fullInterpretation?.detailed?.strengths || [])
      .slice(0, 8),
    blindSpots: traitAnalyses
      .filter(a => a && a.fullInterpretation?.detailed?.potentialChallenges)
      .flatMap(a => a?.fullInterpretation?.detailed?.potentialChallenges || [])
      .slice(0, 6),
  };
}
